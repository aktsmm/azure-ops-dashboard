...
> 旧 TECH-SURVEY.md より統合（2026-02-24）

- **Azure 収集**: z graph query（Azure Resource Graph）採用 — 対象リソースの横断検索が1コマンドで完結
- **図生成**: .drawio（mxfile XML）直接生成を採用 — 追加依存なし、draw.io デスクトップで即編集可能
- **drawio MCP**: ブラウザ拡張前提になりやすいため後段オプション扱い（現行では未使用）
