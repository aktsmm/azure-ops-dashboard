...
_STRINGS: dict[str, dict[str, str]] = {
    ...
    "label.report_targets":     {"ja": "生成:",                         "en": "Generate:"},
    ...
    # --- 出力種別チェックボックス ---
    "opt.inventory_diagram":    {"ja": "📊 インベントリ図",              "en": "📊 Inventory"},
    "opt.network_diagram":      {"ja": "🌐 ネットワーク図",              "en": "🌐 Network"},
    "opt.security_report":      {"ja": "🛡️ セキュリティ",                "en": "🛡️ Security"},
    "opt.cost_report":          {"ja": "💰 コスト",                      "en": "💰 Cost"},

    # --- 図生成オプション ---
    "opt.ai_drawio_layout":     {"ja": "✨ AIレイアウト",                 "en": "✨ AI layout"},
    ...
    # --- View 説明 ---
    "view.diagram_only":        {"ja": "チェックした図を生成",             "en": "Generate checked diagrams"},
    "view.report_only":         {"ja": "チェックしたレポートを生成",       "en": "Generate checked reports"},
    "view.mixed":               {"ja": "チェックした図+レポートを生成",   "en": "Generate checked diagrams + reports"},
    "view.nothing":             {"ja": "生成対象を選択してください",       "en": "Select outputs to generate"},

    # --- ボタン ---
    "btn.collect":              {"ja": "▶ Collect",                     "en": "▶ Collect"},
    "btn.generate_report":      {"ja": "▶ Generate Report",            "en": "▶ Generate Report"},
    "btn.generate":             {"ja": "▶ 生成",                        "en": "▶ Generate"},
    ...
    "log.multi_report_item":    {"ja": "[{index}/{total}] {name} を生成", "en": "[{index}/{total}] Generating {name}"},
    "log.multi_report_hint":    {"ja": "複数レポートは右側の 🛡️/💰 チェックを両方ONにして『Generate Report』を押してください", "en": "For multiple reports, turn on both 🛡️/💰 and click 'Generate Report'."},
    "log.multi_report_template": {"ja": "複数生成モード: {name} は Standard テンプレート（既定セクション）を使用", "en": "Multi-report mode: using Standard template defaults for {name}"},
    "step.summary":            {"ja": "サマリ生成",                "en": "Summary"},
    "status.generating_summary": {"ja": "エグゼクティブサマリを生成中...", "en": "Generating executive summary..."},
    "log.summary_start":       {"ja": "サマリレポート生成: {count} 件のレポートを統合", "en": "Summary report: combining {count} reports"},
    "log.summary_read":        {"ja": "  読み込み: {type} ({path})", "en": "  Read: {type} ({path})"},
    "log.summary_skip_few":    {"ja": "サマリ生成スキップ（レポートが2件未満）", "en": "Summary skipped (fewer than 2 reports)"},
    ...
    "log.limit_clamped":        {"ja": "  ⚠ Max Nodes は {requested} ですが、取得上限の都合で {applied} に丸めます。",
                                 "en": "  ⚠ Max Nodes was {requested}; clamped to {applied} due to collection limits."},
    ...
    "log.ai_drawio_retry":      {"ja": "AI図生成を再試行 ({attempt}/{max})", "en": "Retrying AI diagram generation ({attempt}/{max})"},
    "log.ai_drawio_validation_failed": {"ja": "AI出力がバリデーションに失敗: {count}件", "en": "AI output failed validation: {count} issue(s)"},
    "log.ai_drawio_stats":      {"ja": "AI図生成: ノード反映 {present}/{total}", "en": "AI diagram: nodes included {present}/{total}"},
    "log.ai_drawio_fallback":   {"ja": "AI図生成に失敗したためスクリプト生成にフォールバックします", "en": "AI diagram generation failed; falling back to script."},
