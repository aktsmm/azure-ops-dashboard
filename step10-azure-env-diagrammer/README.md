...
- **Open with** — Auto / Draw.io / VS Code / OS既定 から選択
...
- Windows / Python 3.11+（※ソース実行時。exe 配布で使う場合は Python 不要）
- Azure CLI（`az`）が利用可能
- `az login` 済み
...
- `%APPDATA%\AzureOpsDashboard\templates\`
...
1. **View** を選択（inventory / network / security-report / cost-report）
2. Subscription / Resource Group を入力（任意）
3. レポート系の場合: テンプレート選択 → セクション ON/OFF → 追加指示
4. **▶ Collect** or **▶ Generate Report** ボタンを押す
5. 図の場合: AI レビュー → Proceed で生成 → 自動オープン
6. レポートの場合: AI 生成 → 自動保存 → 自動オープン
